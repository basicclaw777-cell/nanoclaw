/**
 * CJS bridge for llm-router.js — use from ~/Cathedral/ scripts.
 *
 * Usage:
 *   const { llm, llmCall } = require('~/nanoclaw/llm-router.cjs');
...
let config;
try { config = JSON.parse(readFileSync(CONFIG_PATH, 'utf8')); } catch { config = null; }
...
async function callOllama(model, system, prompt, maxTokens, timeoutMs) {
  ...
  try {
    ...
    const res = await fetch(`${ollamaUrl}/api/chat`, {
      ...
      body: JSON.stringify({ model, messages, stream: false, options: { num_predict: maxTokens || 300, temperature: 0.3 } }),
      signal: controller.signal
    });
    clearTimeout(timeout);
    if (!res.ok) { console.error(`[llm-router] Ollama ${res.status} (${model})`); return null; }
    const data = await res.json();
    const content = data.message?.content || null;
    return { content, provider: 'ollama', model, tokens: data.eval_count || 0 };
  } catch (err) {
    clearTimeout(timeout);
    console.error(`[llm-router] Ollama ${err.name === 'AbortError' ? 'timeout' : 'error'} (${model})`);
    return null;
  }
}
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (body.includes('Insufficient Balance') || body.includes('insufficient_quota')) {
        apiDead = true; apiDeadUntil = Date.now() + 3600000;
    ...
    try { appendFileSync(SPEND_LOG, JSON.stringify({ ts: new Date().toISOString(), agent: 'llm-router', model, input_tokens: usage.prompt_tokens || 0, output_tokens: usage.completion_tokens || 0 }) + '\n'); } catch {}
...
async function llm(taskOrTier, system, prompt, opts = {}) {
  ...
  switch (tier.strategy) {
    ...
    case 'local-first':
      result = await callOllama(tier.local_model, system, prompt, maxTokens, tier.timeout_ms);
      if (!result?.content) {
        console.log(`[llm-router] ${callerName}: local miss → API`);
    ...
    case 'api-first':
      result = await callDeepSeek(tier.api_model, system, prompt, maxTokens, tier.timeout_ms);
      if (!result?.content) {
        console.log(`[llm-router] ${callerName}: API miss → local`);
        result = await callOllama(tier.local_model, system, prompt, maxTokens, tier.timeout_ms);
      }
      break;
    case 'api-only':
